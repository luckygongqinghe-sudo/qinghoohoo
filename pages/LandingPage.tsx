...
const LandingPage: React.FC = () => {
  ...
  // 处理标题换行逻辑
  const titleParts = (siteConfig.heroTitle || "数字化、智能化\n结核病筛查模型探索").split('\n');
  ...
  return (
    <div className="bg-[#fbfbfd] scroll-smooth overflow-x-hidden selection:bg-emerald-100 dark:bg-slate-950">
      ...
      <nav className="fixed top-0 w-full glass z-[100] border-b border-black/5 dark:bg-slate-900/70 dark:border-white/5">
        <div className="max-w-7xl mx-auto px-10 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3 group cursor-pointer">
            ...
            <span className="text-lg font-black text-slate-900 dark:text-white tracking-tighter uppercase">{siteConfig.footerBrandName}</span>
          </div>
          ...
          <div className="flex-1 text-center lg:text-left z-10">
            ...
            <h1 className="font-black text-slate-900 dark:text-white leading-[1.05] tracking-tighter mb-6">
               {titleParts.map((line, idx) => (
                  <span key={idx} className={`block ${idx === 0 ? 'text-5xl sm:text-6xl bg-clip-text text-transparent bg-gradient-to-r from-emerald-600 via-blue-600 to-indigo-600 pb-2' : 'text-4xl sm:text-5xl'}`}>
                    {line}
                  </span>
               ))}
            </h1>

            <p className="text-base md:text-lg text-slate-400 dark:text-slate-500 mb-10 leading-relaxed max-w-lg mx-auto lg:mx-0 font-bold opacity-80">
            ...
            <div className="flex flex-col sm:flex-row justify-center lg:justify-start gap-4">
              ...
              <button onClick={() => scrollToSection('features')} className="px-10 py-4 bg-white dark:bg-slate-800 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-sm shadow-sm hover:bg-slate-50 transition-all flex items-center justify-center gap-3">
                <Globe size={16} className="text-slate-300 dark:text-slate-600" /> 特性介绍
              </button>
            </div>
          </div>
          ...
          <div className="flex-1 w-full max-w-lg relative animate-in zoom-in duration-1000">
            <div className="relative aspect-[4/5] bg-slate-950 rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/5">
                ...
                <div className="absolute top-6 right-6 flex flex-col items-end gap-2">
                   ...
                   <span className="text-[8px] font-black uppercase tracking-widest text-emerald-400 bg-emerald-400/10 px-2 py-1 rounded-full">Medical Logic</span>
                </div>
                ...
                <div className="absolute bottom-6 left-6 right-6">
                  <div className="bg-white/5 backdrop-blur-2xl border border-white/10 p-5 rounded-3xl flex items-center gap-4">
                    ...
                    <div><h4 className="text-white font-black text-sm">云端风险算法</h4><p className="text-slate-400 text-[8px] font-black tracking-widest mt-0.5 uppercase">Realtime Analysis</p></div>
                  </div>
                </div>
            </div>
          </div>
      ...
      <section id="features" className="py-24 bg-white dark:bg-slate-900">
        <div className="max-w-7xl mx-auto px-10">
          <div className="mb-16 text-center lg:text-left">
            <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-2 tracking-tighter uppercase">{siteConfig.featuresTitle}</h2>
            <p className="text-base text-slate-400 font-bold opacity-80">{siteConfig.featuresSubtitle}</p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {siteConfig.features.map((f) => (
              <div key={f.id} className="group p-10 bg-[#fbfbfd] dark:bg-slate-800 rounded-[2.5rem] hover:bg-white dark:hover:bg-slate-700 transition-all hover:-translate-y-2 hover:shadow-2xl border border-transparent hover:border-slate-100 dark:hover:border-slate-600">
                ...
                <h3 className="text-lg font-black text-slate-900 dark:text-white mb-3 tracking-tight">{f.title}</h3>
                <p className="text-slate-400 dark:text-slate-500 text-xs leading-relaxed font-bold opacity-80">{f.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="about" className="py-24 bg-[#fbfbfd] dark:bg-slate-950">
        <div className="max-w-7xl mx-auto px-10">
          <div className="bg-white dark:bg-slate-900 rounded-[3.5rem] p-10 md:p-14 shadow-xl flex flex-col lg:flex-row gap-12 items-center border border-slate-100 dark:border-slate-800">
            <div className="lg:w-1/2">
              <span className="text-emerald-600 font-black tracking-widest text-[9px] uppercase block mb-4 tracking-[0.3em]">Compliance & Safety</span>
              <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-10 tracking-tighter leading-tight">{siteConfig.aboutTitle}</h2>
              <div className="space-y-6">
                {siteConfig.aboutItems.map((item) => (
                  <div key={item.id} className="flex gap-4 group">
                    ...
                    <div>
                      <h4 className="text-base font-black text-slate-900 dark:text-white mb-0.5">{item.title}</h4>
                      <p className="text-slate-400 dark:text-slate-500 text-xs font-bold leading-relaxed">{item.description}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="lg:w-1/2 w-full">
               <div className="w-full aspect-video bg-slate-950 rounded-[2.5rem] p-8 flex flex-col justify-between text-white overflow-hidden shadow-2xl relative group">
                  <div className="flex justify-between items-start">
                     ...
                     <span className="text-[8px] font-black uppercase tracking-widest bg-emerald-400/10 text-emerald-400 px-3 py-1 rounded-full border border-emerald-400/20">Data Protection</span>
                  </div>
                  <div>
                     <div className="text-5xl font-black mb-2 tracking-tighter">AES-256</div>
                     <p className="text-slate-400 text-[9px] font-black uppercase tracking-widest">Medical Level Encryption</p>
                  </div>
               </div>
            </div>
          </div>
