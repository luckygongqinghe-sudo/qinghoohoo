...
const CaseSummaryPage: React.FC = () => {
  ...
  const exportDetailedCSV = () => {
    ...
    const headers = [
      ...
      '既往史', '临床症状', '暴露史', '影像学特征', '标准路径评分', 'QFT结果', '痰涂片', '痰培养', 
      '最终综合评分', '风险等级', '处置建议', 'AI推理逻辑', 'AI融合分值', 'AI异常信号', 'AI专家建议', 'AI置信度'
    ];
    ...
    link.href = url;
    link.download = `TB_Scan_Comparison_Data_${new Date().toISOString().split('T')[0]}.csv`;
    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-32 relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border-2 border-slate-100 dark:border-slate-800 shadow-sm h-[380px] flex flex-col">
          <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2"><PieChartIcon size={16} className="text-emerald-500" /> 风险分层分布统计</h3>
          <div className="flex-1">
            ...
            ) : <div className="h-full flex items-center justify-center text-slate-300 font-bold">无临床数据可统计</div>}
          </div>
        </div>
        <div className="bg-white dark:bg-slate-900 p-8 rounded-[40px] border-2 border-slate-100 dark:border-slate-800 shadow-sm h-[380px] flex flex-col">
          <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-6 flex items-center gap-2"><BarChart3 size={16} className="text-blue-500" /> 高频临床症状透视</h3>
          <div className="flex-1">
            ...
            ) : <div className="h-full flex items-center justify-center text-slate-300 font-bold">无记录</div>}
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 text-[10px] font-black uppercase tracking-widest text-slate-400">
                ...
                <th className="py-6 px-6">受检者详情与诊断标签</th>
                <th className="py-6 px-6">协同推理引擎 (AI)</th>
                <th className="py-6 px-6 text-center">综合分值</th>
                <th className="py-6 px-8 text-right">操作审计</th>
              </tr>
            </thead>
                    ...
                    <td className="py-6 px-6">
                      {c.aiInference ? (
                        <div className="flex items-center gap-2 text-indigo-600 dark:text-indigo-400">
                          ...
                          <span className="text-[10px] font-black uppercase tracking-widest">Qwen3 Max 融合推理完成</span>
                        </div>
                      ) : (
                        <div className="text-[10px] font-bold text-slate-300 uppercase tracking-widest">仅专家路径计算</div>
                      )}
                    </td>
                      ...
                      <div className="flex items-center justify-end gap-3">
                        ...
                        {(isAdmin || currentUser?.id === c.creatorId) && (
                          <>
                            ...
                            <button onClick={(e) => { e.stopPropagation(); if(window.confirm('确认物理删除该病例？')) deleteCases([c.id]); }} className="p-2 text-slate-300 hover:text-rose-500"><Trash2 size={18} /></button>
                          </>
                        )}
                      </div>
                    </td>
                        ...
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-10">
                          <div className="space-y-6">
                            <div className="flex items-center gap-3">
                              ...
                              <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">临床路径结构化指标 (基准比对)</h4>
                            </div>
                            <div className="grid grid-cols-2 gap-x-12 gap-y-4 bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-inner border border-slate-100 dark:border-slate-700">
                              <DetailItem label="BMI 指数" value={`${c.bmi} (${c.bmi < 18.5 ? '偏低' : c.bmi > 24 ? '超重' : '正常'})`} />
                              <DetailItem label="影像学主要特征" value={c.ctFeature} />
                              <DetailItem label="病原学组合结果" value={`QFT:${c.qftResult} / 涂片:${c.smearResult} / 培养:${c.cultureResult}`} />
                              <DetailItem label="临床症状识别" value={c.symptoms.join(', ') || '无明显核心症状'} />
                              <DetailItem label="流行病风险背景" value={c.history.join(', ') || '无特定病史记录'} />
                              <DetailItem label="档案录入责任人" value={c.creatorName} />
                            </div>
                            <div className="p-6 bg-slate-950 rounded-2xl border border-white/5">
                               <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">临床处置方案 (标准建议)</p>
                               <p className="text-xs text-slate-300 italic">“{c.suggestion}”</p>
                            </div>
                          </div>

                          <div className="bg-slate-950 rounded-[2.5rem] p-8 space-y-8 border border-white/5 shadow-2xl relative overflow-hidden">
                            <div className="absolute -top-10 -right-10 w-40 h-40 bg-indigo-600/10 blur-[60px] rounded-full"></div>
                            <div className="flex items-center justify-between relative z-10">
                              <div className="flex items-center gap-3">
                                <BrainCircuit size={20} className="text-indigo-400" />
                                <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em]">Synergy 2.0 AI 推理分析报告</h4>
                              </div>
                              {c.aiInference && (
                            ...
                            {c.aiInference ? (
                              <div className="space-y-6 relative z-10">
                                <div className="bg-white/5 p-6 rounded-3xl border border-white/5 space-y-3 shadow-inner">
                                  <div className="flex items-center gap-2 text-indigo-400">
                                    <Zap size={14} />
                                    <span className="text-[9px] font-black uppercase tracking-widest">可解释性深度推理链</span>
                                  </div>
                                ...
                                {c.aiInference.anomalies.length > 0 && (
                                  <div className="space-y-3">
                                    <div className="flex items-center gap-2 text-rose-500">
                                      ...
                                      <span className="text-[9px] font-black uppercase tracking-widest">识别到的异常冲突/高危风险</span>
                                    </div>
                                ...
                                <div className="bg-emerald-500/10 p-6 rounded-3xl border border-emerald-500/20 space-y-3 shadow-lg">
                                  <div className="flex items-center gap-2 text-emerald-400">
                                    <Activity size={14} />
                                    <span className="text-[9px] font-black uppercase tracking-widest">AI 协同处置方案</span>
                                  </div>
                                ...
                                <div className="flex items-center justify-between pt-6 border-t border-white/5">
                                  <div className="flex flex-col">
                                    <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Neural Fusion Score</span>
                                    <span className="text-3xl font-black text-indigo-400 tracking-tighter">{c.aiInference.fusionScore}</span>
                                  </div>
                                  <div className="flex flex-col items-end">
                                    <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest">Guideline Path Score</span>
                                    <span className="text-2xl font-black text-slate-400 tracking-tighter">{c.ctScore + (c.totalScore - c.ctScore)}</span>
                                  </div>
                                </div>
                              </div>
                            ) : (
                              <div className="py-24 text-center space-y-4">
                                <Zap size={48} className="text-slate-800 mx-auto" />
                                <p className="text-[11px] text-slate-600 font-black uppercase tracking-[0.3em]">该病例未进行 AI 决策融合分析</p>
                                <button 
                                  onClick={(e) => { e.stopPropagation(); navigate(`/dashboard/cases?edit=${c.id}`); }}
                                  className="px-8 py-3 bg-indigo-600 text-white rounded-full text-[10px] font-black shadow-xl hover:scale-105 transition-all"
                                >
                                  启动协同诊断引擎 (Synergy v2.0)
                                </button>
                              </div>
                            )}
                          </div>
              ...
              {filteredCases.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-40 text-center">
                    <PieChartIcon size={64} className="text-slate-100 dark:text-slate-900 mx-auto mb-6" />
                    <p className="text-slate-400 font-black text-sm uppercase tracking-[0.2em]">未找到符合条件的临床诊断案例</p>
                  </td>
                </tr>
              )}
            </tbody>
